...
Modified from SLOPER4D
===========================================================

**Original Project**: SLOPER4D: A Scene-Aware Dataset for Global 4D Human Pose Estimation in Urban Environments
**Source URL**: https://github.com/climbingdaily/SLOPER4D
...
def fix_points_num(points: np.array, num_points: int):
    ...
    if origin_num_points < num_points:
        ...
        num_zeros = num_points - origin_num_points
        zeros = np.zeros((num_zeros, 3))
        res = np.vstack((points, zeros))
    else:
...
class SLOPER4D_Dataset(Dataset):
    ...
    def load_3d_data(self, data, person='second_person', points_num=1024):
        ...
        self.generate_sshmr_data()


    def fliter_nearby_points(self, new_pc, pc, fliter_threshold=0.05):
        dist1, dist2 = uni_utils.my_chamfer_distance(new_pc, pc, is_return_dist=True)
        selected_mask = dist1 < fliter_threshold
        selcted_begin_pos = new_pc[selected_mask]
        return selcted_begin_pos

    def generate_sshmr_data(self):
        valid_tmp_mask=torch.sum(torch.abs(torch.tensor(self.human_points)),dim=-1)>1e-6
        valid_pc_num=torch.sum(valid_tmp_mask,dim=-1)

        filter_mask=valid_pc_num>100
        fliter_human_points=torch.tensor(self.human_points[filter_mask])
        fliter_smpl_pose=torch.tensor(self.smpl_pose[filter_mask])
        fliter_global_trans=torch.tensor(self.global_trans[filter_mask])
        fliter_beta=torch.tensor(self.betas).unsqueeze(0).repeat(fliter_smpl_pose.shape[0],1)

        d,j=uni_utils.get_smpl_result_no_humanmodel(fliter_smpl_pose.cuda(),fliter_beta.cuda(),fliter_global_trans.cuda(),model_root='../model/smpl/pytorch/models/')

        fliter_human_points=uni_utils.coordinate_transform(fliter_human_points)


        j=uni_utils.coordinate_transform(j)

        new_pc=[]
        for i in range(0,fliter_human_points.shape[0]):
            tmp_pos = fliter_human_points[i].clone()
            cdsit = torch.cdist(tmp_pos, tmp_pos)
            min_dist = torch.topk(cdsit, 4, dim=-1, largest=False).values
            mask = min_dist[:, 1] < 0.09

            tmp_pos = tmp_pos[mask]

            valid_mask=torch.sum(torch.abs(tmp_pos),dim=-1)>1e-6
            tmp_pos=tmp_pos[valid_mask]

            new_pc.append(tmp_pos)

        final_data=[]
        data_len=[]
        for i in range(0,fliter_human_points.shape[0]):
            mid_pos = new_pc[i].clone()
            final_pos = mid_pos
            mid_mean = torch.mean(mid_pos, dim=0)

            gap_threshold=0.08
            chamfer_threshold=0.05
            if(i==0):
                end_pos = new_pc[i + 1].clone()
                end_mean=torch.mean(end_pos,dim=0)
                if(torch.norm(mid_mean-end_mean)<gap_threshold):
                    final_pos=torch.cat((final_pos,end_pos),dim=0)
                elif(torch.norm(mid_mean-end_mean)<0.2):
                    fliter_pos = self.fliter_nearby_points(end_pos, mid_pos)
                    final_pos = torch.cat((final_pos, fliter_pos), dim=0)

            elif(i==(fliter_human_points.shape[0]-1)):
                begin_pos = new_pc[i - 1].clone()
                begin_mean=torch.mean(begin_pos,dim=0)
                if(torch.norm(begin_mean-mid_mean)<gap_threshold):
                    final_pos=torch.cat((final_pos,begin_pos),dim=0)
                elif (torch.norm(mid_mean - begin_mean) < 0.2):
                    fliter_pos = self.fliter_nearby_points(begin_pos, mid_pos)
                    final_pos = torch.cat((final_pos, fliter_pos), dim=0)
            else:
                ...
                if(torch.norm(begin_mean-mid_mean)<gap_threshold):
                    ...
                    fliter_pos=self.fliter_nearby_points(begin_pos,mid_pos)
                    final_pos = torch.cat((final_pos, fliter_pos), dim=0)
                elif (torch.norm(mid_mean - begin_mean) < 0.3):
                    fliter_pos = self.fliter_nearby_points(begin_pos, mid_pos,fliter_threshold=0.03)
                    final_pos = torch.cat((final_pos, fliter_pos), dim=0)
                    #visual_utils.plot_mutil_fig([final_pos, mid_pos])
                ...
                if(torch.norm(mid_mean-end_mean)<gap_threshold):
                    ...
                    final_pos = torch.cat((final_pos, fliter_pos), dim=0)
                elif (torch.norm(mid_mean - end_mean) < 0.3):
                    fliter_pos = self.fliter_nearby_points(end_pos, mid_pos,fliter_threshold=0.03)

                    final_pos = torch.cat((final_pos, fliter_pos), dim=0)

            data_len.append(final_pos.shape[0])
            if(final_pos.shape[0]<1024):
                final_pos=torch.cat((final_pos,torch.zeros((1024-final_pos.shape[0],3))),dim=0)
            else:
                final_pos=final_pos[:1024]

            tmp_data={
                "point_cloud":final_pos.detach().cpu().numpy(),
                "gt_pose":fliter_smpl_pose[i].detach().cpu().numpy(),
                "gt_shape":fliter_beta[i].detach().cpu().numpy(),
                "camLoc":fliter_global_trans[i].detach().cpu().numpy(),
                "gt_joints3d":j[i].detach().cpu().numpy(),
            }
            final_data.append(tmp_data)
        prepath= os.path.dirname(self.pkl_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pkl_file', type=str,
                        default='../data/SLOPER4D/seq009_running_002/seq009_running_002_labels.pkl',
    ...
    file_list=['../data/SLOPER4D/seq002_football_001/seq002_football_001_labels.pkl',
               '../data/SLOPER4D/seq003_street_002/seq003_street_002_labels.pkl',
               '../data/SLOPER4D/seq005_library_002/seq005_library_002_labels.pkl',
               '../data/SLOPER4D/seq007_garden_001/seq007_garden_001_labels.pkl',
               '../data/SLOPER4D/seq008_running_001/seq008_running_001_labels.pkl',
               '../data/SLOPER4D/seq009_running_002/seq009_running_002_labels.pkl',
    ...
    for file in file_list:
        ...
        dataset = SLOPER4D_Dataset(args.pkl_file,
                                   return_torch=False,
                                   fix_pts_num=True)
        final_dataset=final_dataset+dataset.result_data
        print(len(final_dataset))

    np.random.seed(42)
    test_index=np.random.choice(len(final_dataset),int(len(final_dataset)*0.1),replace=False)

    test_size = int(len(final_dataset) * 0.3)
    test_indices = np.random.choice(len(final_dataset), test_size, replace=False)

    # 创建测试集和训练集
    test_set = [final_dataset[i] for i in test_indices]
    train_set = [item for i, item in enumerate(final_dataset) if i not in test_indices]

    # 打印信息
    print(f"Length of Training Dataset: {len(train_set)}")
    print(f"Length of Testing Dataset: {len(test_set)}")
    np.save("../data/SLOPER4D/ntrain_data.npy",train_set)
    np.save("../data/SLOPER4D/ntest_data.npy",test_set)
